...
For my component and representation, I will be using some data structures from the OSU components. These include three maps that will contain data about the book's title, whether a book has been read, and the book's author. Books has the title and genre, status has the title and read/unread, and authors has the title and its author. From this, I will be able do implement my kernel methods in an efficient matter, as maps involve various methods like add, remove, and hasKey to easily access and changes its elements. It overall keeps the representation clean and flexible.

Convention: Every book would have exactly one genre, one author, and one read/unread status. No book entry can be missing from any of the three maps. Each title appears at most once in each map, and no key/value is null.
